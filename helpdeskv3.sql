...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `Del_usuario`(IN `pId` INT(11))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `llamado_borrados`()
BEGIN
SELECT * FROM del_usuarios;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `new_procedure`(IN `xusu_id` INT)
BEGIN
	UPDATE tm_usuario SET est='0', fech_elim = now() where usu_id = xusu_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_d_usuario`(IN `xusu_id` INT)
BEGIN
	UPDATE tm_usuario SET est='0', fech_elim = now() where usu_id = xusu_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_graficoxusuario`(IN `xusu_id` INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_i_ticket`(IN `xusu_id` INT, IN `xcat_id` INT, IN `xtick_titulo` VARCHAR(100), IN `xtick_descrip` VARCHAR(9000))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_i_ticket_d`(IN `xtick_id` INT, IN `xusu_id` INT, IN `xtickd_descrip` VARCHAR(9000))
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_i_usuario`(IN `xusu_nom` VARCHAR(150), IN `xsu_ape` VARCHAR(150), IN `xusu_correo` VARCHAR(150), IN `xusu_pass` VARCHAR(150), IN `xrol_id` INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_listar_tick_x_usu`(IN `xusu_id` INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_l_ticket`()
BEGIN
	SELECT
                tm_ticket.tick_id,
                tm_ticket.usu_id,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_l_ticketdetalle_xticket`(IN `xtick_id` INT)
BEGIN
	SELECT
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_l_ticket_x_id`(IN `xtick_id` INT)
BEGIN
	SELECT 
                tm_ticket.tick_id,
                tm_ticket.usu_id,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_l_usuario02`(IN `xusu_id` INT)
BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_totalticketabiertosxusuario`(IN `xusu_id` INT)
BEGIN
	SELECT COUNT(*) TOTAL FROM tm_ticket WHERE usu_id = xusu_id AND tick_estado = 'Abierto';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_totalticketcerradosxusuario`(IN `xusu_id` INT)
BEGIN
	SELECT COUNT(*) TOTAL FROM tm_ticket WHERE usu_id = xusu_id AND tick_estado = 'Cerrado';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_totalticketxusuario`(IN `xusu_id` INT)
BEGIN
	SELECT COUNT(*) TOTAL FROM tm_ticket WHERE usu_id = xusu_id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_t_ticket`()
BEGIN
	SELECT COUNT(*) TOTAL FROM tm_ticket;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_t_ticketgrafico`()
BEGIN
	SELECT tm_categoria.cat_nom as nom,COUNT(*) AS total
                    FROM tm_ticket JOIN 
                    tm_categoria ON tm_ticket.cat_id = tm_categoria.cat_id
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_t_ticket_abierto`()
BEGIN
	SELECT COUNT(*) TOTAL FROM tm_ticket WHERE tick_estado = 'Abierto';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_t_ticket_cerrado`()
BEGIN
	SELECT COUNT(*) TOTAL FROM tm_ticket WHERE  tick_estado = 'Cerrado';
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_u_ticket`(IN `xtick_id` INT)
BEGIN
	update tm_ticket 
                  set	
                  tick_estado = 'Cerrado'
